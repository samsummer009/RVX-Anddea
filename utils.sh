...
_req() {
	...
	shift 2
	if [ "$op" = - ]; then
		if ! curl -L -c "$TEMP_DIR/cookie.txt" -b "$TEMP_DIR/cookie.txt" --connect-timeout 30 --retry 3 --retry-delay 5 --fail -s -S "$@" "$ip"; then
...
get_patch_last_supported_ver() {
	local list_patches=$1 pkg_name=$2 inc_sel=$3 _exc_sel=$4 _exclusive=$5 cli_jar=$6 patches_jar=$7
	local op
	if [ "$inc_sel" ]; then
...
patches_list() {
	local cli_jar=$1 patches_jar=$2 pkg_name=$3 op
	if ! op=$(java -jar "$cli_jar" list-patches -f "$pkg_name" -d -i -o -p -u -v --patches="$patches_jar" 2>&1); then
...
build_rv() {
		...
		for dl_p in archive apkmirror uptodown; do
			...
			if ! dl_${dl_p} "${args[${dl_p}_dlurl]}" "$version" "$stock_apk" "$arch" "${args[dpi]}" "$get_latest_ver"; then
				...
				if [ "$dl_p" = "archive" ]; then
					...
					pr "Retrying with version '${version}'"
					if dl_${dl_p} "${args[${dl_p}_dlurl]}" "$version" "$stock_apk" "$arch" "${args[dpi]}" true; then
						break
					fi
				fi
				continue
	...
	if [ "${args[patcher_args]}" ] && [ "${args[patcher_args]}" != "false" ]; then p_patcher_args+=("${args[patcher_args]}"); fi
